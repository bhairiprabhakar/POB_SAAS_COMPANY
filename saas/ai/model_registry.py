...
stored in the platform control-plane database (saas/platform_db.py) so a
superadmin can change either WITHOUT a restart. This is the merged home of
the legacy app/ai/model_registry.py.

Two small tables (see saas/platform_db.py _PLATFORM_DDL):
...
saas/ai/gemini_extraction.py (_category_for / _select_model). env_attr names
the saas.config attribute whose value is that category's ".env default".
"""
import re
from .. import config
...
# broken/absent table can never take down extraction. These tables live in the
# platform control-plane DB (see saas/platform_db.py) -- model routing and
# pricing are platform-wide superadmin config, not per-tenant data.
# ─────────────────────────────────────────────────────────────────────────────
def _conn():
