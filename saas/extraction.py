...
provides duplicate-detection hashing. Merged home of the legacy
app/extraction.py; the mapping logic is unchanged -- Gemini is prompted (see
saas/ai/gemini_extraction.py) to emit the same Areas -> Stores -> Items shape,
so this mapper is identical.
"""
...
def call_ocr_extraction(file_path: str, file_type: str, original_name: str,
                         company_id=None, upload_id=None, division_id=None,
                         user_id=None) -> dict:
    """
    ...
    attribute AI usage cost to the right company (see saas/ai/pricing.py and
    the superadmin AI Costing dashboard) -- omitting them just means that
    call's cost isn't logged, extraction itself is unaffected. division_id
    tenant-annotates the logged usage row; user_id / original_name feed the
    costing dashboard's per-user / per-document views.
    """
    parsed = call_gemini_extraction(file_path, file_type, original_name,
                                     company_id=company_id, upload_id=upload_id,
                                     division_id=division_id, user_id=user_id)
