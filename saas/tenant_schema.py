...
-- ── Remove the Statements / Statement Credits feature (3.21.0) ──────────────
-- The statement-document extraction domain (uploads, extractions, parties,
-- items, manual_verifications) and its credits wallet/request ledger
-- (credits, credit_transactions, credit_requests) are being decommissioned
-- project-wide -- this product does not want to capture statements. Drop the
-- tables (children before parents for FK safety; CASCADE as a backstop) and
-- remove the statement.* permission codes and their role grants from every
-- already-provisioned tenant.
DELETE FROM role_permissions WHERE permission_code LIKE 'statement.%';
DELETE FROM permissions WHERE code LIKE 'statement.%';
