...
direct web link to a PDF/image.
...
def download_remote_document(raw_url: str):
    ...
    if content_length and int(content_length) > MAX_UPLOAD_SIZE:
        resp.close()
        raise RemoteFetchError(f"File is too large ({int(content_length)//1024//1024} MB). Max is 100 MB.")
        ...
        with os.fdopen(fd, "wb") as out:
            ...
            for chunk in resp.iter_content(chunk_size=65536):
                ...
                if total > MAX_UPLOAD_SIZE:
                    raise RemoteFetchError("File exceeded the 100 MB limit while downloading.")
                if first_chunk:
