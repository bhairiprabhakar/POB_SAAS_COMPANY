...
old pdfplumber + PaddleOCR + custom-router pipeline entirely.

Public entry point kept close to the original app.py:

    call_ocr_extraction(file_path, file_type, original_name, company_id=None, upload_id=None) -> dict

...so app/helpers.py::save_extraction() and the /upload route needed only a
couple of extra (optional, backward-compatible) keyword args added -- see
app/extraction.py.
...
def cleanup_orphaned_gemini_files(older_than_seconds: int = 3600) -> int:
    ...
    _upload_and_extract() already deletes its own upload as soon as
    extraction finishes, so in the normal case this finds nothing. This
    exists as a safety net for the case that motivated it: a process crash,
    OOM kill, or unhandled exception between client.files.upload() and the
    try/finally in _upload_and_extract() would otherwise leave that file
    orphaned until Gemini's own ~48h auto-expiry. Run once a day from
    saas/scheduler.py's daily pass (project-wide, not per-tenant -- Gemini
    File API storage isn't tenant-scoped). Returns the number deleted."""
    import datetime
    import logging

    log = logging.getLogger("app.ai.gemini_extraction")
    client = _get_client()
...
def _upload_and_extract(file_path: str, mime_type: str, model_name: str):
    ...
    try:
        ...
        try:
            ...
            import logging
            logging.getLogger("app.ai.gemini_extraction").warning(
...
def _select_model(file_path: str, mime_type: str) -> str:
    ...
    app/ai/model_registry.py resolve_model()). Returns a model ID string.
    """
...
def call_gemini_extraction(file_path: str, file_type: str, original_name: str,
    ...
    (app/helpers.py's upload flow) routes the document to Manual Verification,
    exactly like the original OCR pipeline's error handling.
    ...
    actually used) to ai_usage_log for the superadmin AI Costing dashboard --
    both optional so this function still works standalone (e.g. from a
    script) without a DB available.
    """
...
def _log_ai_usage(company_id, upload_id, model_name: str, usage: dict, chunk_count: int):
    ...
    try:
        ...
        c.execute("""INSERT INTO ai_usage_log
            (upload_id, company_id, model_name, input_tokens, output_tokens,
             thinking_tokens, chunk_count, cost_usd, cost_inr)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
            (upload_id, company_id, model_name,
